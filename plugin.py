...
def run(bk):
    ...
    viewer_home = SCRIPT_DIR
    epub_home = os.path.join(viewer_home, 'bibi-bookshelf')
    os.makedirs(epub_home, exist_ok = True)
    
    bookdir = tempfile.mkdtemp()
    ...
    epub_fpath = os.path.join(epub_home, bookdir_name + '.epub')
    epub_zip_up_book_contents(bookdir, epub_fpath)
    ...
    # done with temporary epub file so clean up after yourself
    os.remove(epub_fpath)
    
    print("Bibi Reader Session Complete")
    bk.savePrefs(prefs)
